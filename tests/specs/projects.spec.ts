...
test.describe('Feature: Project Management', () => {
  ...
  test('Project Wizard: Can brainstorm and create a project', async ({ page }) => {
    ...
    await expect(page.getByTestId('wizard-title-input')).toHaveValue('The Quantum Paradox', {
      timeout: 5000,
    });

    // Brainstorm Style
    await page.getByTestId('wizard-brainstorm-style').click();
    await expect(page.getByTestId('wizard-style-input')).toHaveValue('Hard Sci-Fi, Gritty', {
      timeout: 5000,
    });
