...
function updateCircuitBreaker(provider: AIProvider, success: boolean): void {
  ...
  if (success) {
    ...
    if (breaker.failureCount >= CIRCUIT_BREAKER_THRESHOLD && !breaker.isOpen) {
      breaker.isOpen = true;
      console.warn(`[HealthService] Circuit breaker opened for ${provider} - ${breaker.failureCount} consecutive failures`);
    }
  }
}
...
export async function checkProviderHealth(
  ...
  if (!shouldAttemptRecovery(provider)) {
    console.log(`[HealthService] Skipping ${provider} - circuit breaker open, waiting for recovery window`);
    return;
  }
  ...
  const result = await performHealthCheck(provider, modelName, providerConfig.apiKey);
...
export async function checkAllProvidersHealth(userId: string = 'system'): Promise<void> {
  ...
  await Promise.allSettled(
    enabledProviders.map(provider => checkProviderHealth(provider, userId))
  );
...
export function startHealthMonitoring(userId: string = 'system'): void {
  ...
  console.log(`[HealthService] Starting periodic health monitoring (interval: ${HEALTH_CHECK_INTERVAL_MS / 1000}s)`);
...
export async function getHealthReport(): Promise<{
  ...
  const providerReports = providers.map(provider => {
    const health = healthRecords.find(h => h.provider === provider) || null;
