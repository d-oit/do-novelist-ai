...
export function useWorldBuilding(projectId?: string): UseWorldBuildingReturn {
  ...
  const initializeWorldBuilding = useCallback(async (targetProjectId: string): Promise<void> => {
    setIsLoading(true);
    setError(null);
  ...
  const createLocation = useCallback(async (data: Partial<Location>): Promise<Location | null> => {
    if ((projectId?.length ?? 0) === 0 && (worldBuilding?.projectId?.length ?? 0) === 0) return null;
  ...
  const updateLocation = useCallback(async (id: string, updates: Partial<Location>): Promise<Location | null> => {
    try {
  ...
  const createCulture = useCallback(async (data: Partial<Culture>): Promise<Culture | null> => {
    if ((projectId?.length ?? 0) === 0 && (worldBuilding?.projectId?.length ?? 0) === 0) return null;
  ...
  const updateCulture = useCallback(async (id: string, updates: Partial<Culture>): Promise<Culture | null> => {
    try {
  ...
  const searchElements = useCallback(async (filters: WorldBuildingFilters): Promise<void> => {
    if ((projectId?.length ?? 0) === 0 && (worldBuilding?.projectId?.length ?? 0) === 0) return;
    ...
    try {
      const results = await worldBuildingService.searchWorldElements(targetProjectId as string, filters);
