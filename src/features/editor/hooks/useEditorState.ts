...
export function useEditorState() {
    ...
    const actions = {
        setChapter: useCallback((summary: string, content: string) =>
        ...
        setGeneratingImage: useCallback((isGenerating: boolean) =>
            dispatch({ type: 'SET_GENERATING_IMAGE', payload: isGenerating }), []),

        setShowVersionHistory: useCallback((show: boolean) =>
            dispatch({ type: 'TOGGLE_VERSION_HISTORY', payload: show }), []),

        setShowAnalytics: useCallback((show: boolean) =>
            dispatch({ type: 'TOGGLE_ANALYTICS', payload: show }), []),

        showComparison: useCallback((v1: any, v2: any) =>
            dispatch({ type: 'SHOW_COMPARISON', payload: [v1, v2] }), []),

        closeComparison: useCallback(() =>
            dispatch({ type: 'CLOSE_COMPARISON' }), []),

        updateRefineSettings: useCallback((settings: Partial<RefineOptions>) =>
