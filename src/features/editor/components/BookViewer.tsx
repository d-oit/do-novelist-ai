...
const BookViewer: React.FC<BookViewerProps> = React.memo(
  ({
    project,
    selectedChapterId,
    onSelectChapter,
    ...
    const currentWordCount = useMemo(
      () => getWordCount(state.content),
      [state.content, getWordCount],
    );
    const readingTime = useMemo(
      () => Math.max(1, Math.ceil(currentWordCount / 230)),
      [currentWordCount],
    );

    const handleGenerateIllustration = useCallback((): void => {
    ...
    const handleRestoreVersion = useCallback(
      ...
      [onUpdateChapter, actions, saveVersion],
    );
            ...
            >
              ...
              {selectedChapter.illustration != null && (
                ...
                >
                  ...
                  >
                    <RefreshCw
                      className={cn('h-4 w-4', state.isGeneratingImage && 'animate-spin')}
                    />
                  </button>
                </div>
              )}
