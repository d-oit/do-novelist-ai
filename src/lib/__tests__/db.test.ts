...
describe('Database Library', () => {
    ...
    describe('Initialization', () => {
        it('should initialize cloud client if configured', async () => {
            localStorage.setItem('novelist_db_config', JSON.stringify({
                url: 'libsql://test.db',
                authToken: 'token',
                useCloud: true
            }));
            ...
            expect(mockExecute).toHaveBeenCalledWith(expect.stringContaining('CREATE TABLE IF NOT EXISTS projects'));
        });
